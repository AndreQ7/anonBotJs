...
bot.command("start", async (ctx) =>{
    ...
    if (agr == undefined){
      ...
      let shareurl = ("https://t.me/share/url?url=t.me/test_xen_bot?start="+user_id);
      const startKeyboard = new InlineKeyboard().url("Share", shareurl);
      await ctx.reply(`<i>Привет! Поделитесь этой <a href="t.me/test_xen_bot?start=+${user_id}">ссылкой</a>,\nчтобы получать анонимные сообщения!</i>`, {reply_markup: startKeyboard, parse_mode: 'HTML'});
    }else{
        let user_id = agr;
